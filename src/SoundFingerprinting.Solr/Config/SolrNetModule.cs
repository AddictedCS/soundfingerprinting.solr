namespace SoundFingerprinting.Solr.Config
{
    ...
    public class SolrNetModule : NinjectModule
    {
        ...
        private readonly IList<SolrCore> cores = new List<SolrCore>();
        ...
        public SolrNetModule(IList<SolrServer> solrServers)
        {
            AddCoresFromConfig(solrServers);
        }

        private void AddCoresFromConfig(IList<SolrServer> solrServers)
        {
            if (solrServers == null)
            {
                return;
        ...
        private static SolrCore GetCoreFrom(SolrServer server)
        {
        ...
        private static string GetCoreUrl(SolrServer server)
        {
            var url = server.Url;
            if (string.IsNullOrEmpty(url))
                throw new Exception("Core url missing in SolrNet core configuration");
            return url;
        }

        private static Type GetCoreDocumentType(SolrServer server)
        {
            var documentType = server.DocumentType;

            if (string.IsNullOrEmpty(documentType))
                throw new Exception("Document type missing in SolrNet core configuration");
            ...
            {
                throw new Exception(string.Format("Error getting document type '{0}'", documentType), e);
            }

            if (type == null)
                throw new Exception(string.Format("Error getting document type '{0}'", documentType));
